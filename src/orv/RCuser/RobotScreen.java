 ...
 *
 */
public class RobotScreen implements Runnable {
	...
	public int[] getZoneScreenshot(int x, int y, int width, int height) {
		// Get the screen from the zone 
		Rectangle screenRect = new Rectangle(x, y, width, height);
		BufferedImage colorImage = robot.createScreenCapture(screenRect);
			
		// Scale :
		/*BufferedImage cImgScaled = Scalr.resize(colorImage,
			            Scalr.Method.BALANCED,
			            Scalr.Mode.AUTOMATIC,
			            width,
			            (int) Math.round(height/1.04));
				
		colorImage = Scalr.resize(cImgScaled,
			            Scalr.Method.SPEED,
			            Scalr.Mode.AUTOMATIC,
			            width,
			            height);
